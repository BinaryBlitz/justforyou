class Export
  ...
  def kitchen
    ...
    CSV.open(file, "wb") do |csv|
      ...
      @deliveries.each do |delivery|
        substitutions = delivery.purchase.user.substitutions.collect { |sub| sub.product.name }.join(", ")
        block = "#{delivery.purchase.program.block.name}/#{delivery.day_position}"
        csv << [ delivery.id, delivery.purchase.user.full_name, substitutions, block ]
      end
    end
  end
  ...
  def courier
    ...
    CSV.open(file, "wb") do |csv|
      ...
      @deliveries.each do |delivery|
        csv << [
          delivery.id, delivery.purchase.user.full_name, delivery.purchase.user.phone_number,
